...
//        int[] arr = {20,10,4,12};
////        전동적인 방식의 데이터 접근방법 : 메모리주소접근 (값변경가능)
...
////        stream이라는 arr사본을 만듬. a변수에 담아 출력
//
////        스트림의 생성 : .stream()
...
//         myList.add("python");
//         myList.add("C++");
//        Stream<String> stream1 = myList.stream();   //stream객체 리턴
//        String[] myArr = {"java","python","C++"};
//        Stream<String> stream2 = Arrays.stream(myArr);
////         원시자료형을 위한 stream객체가 별도로 존재
//        int[] intArr = {10,20,30,40,50};
//        IntStream stream3 = Arrays.stream(intArr);      //Stream<int> 안됨!

////        stream의 중개연산(변환) : filter, map, sorted, distinct, mapToInt
//        int[] intArr = {10, 10, 30, 40, 50};
//        Arrays.stream(intArr).filter(a -> a>=30);   //리턴값 스트림객체
////        int[] result = Arrays.stream(intArr).filter(a -> a>=30);  //이렇게 할수없음
...
//        int[] result1 = Arrays.stream(arr).filter(a->a%2!=0).toArray();
//        System.out.println(Arrays.toString(result1));
////        실습2. arr에서 홀수만 걸러서 해당 홀수의 제곱값을 담은 배열출력
//        int[] result2 = Arrays.stream(arr).filter(a->a%2!=0).map(b->b*b).toArray();
//        System.out.println(Arrays.toString(result2));
////        실습3. arr에서 홀수만 걸러 제곱값을 구하고, 해당숫자값을 오름차순한 순서로 배열출력
//        int[] result3 = Arrays.stream(arr).filter(a->a%2!=0).map(b->b*b).sorted().toArray();
...
//        System.out.println(total);
//        long count = Arrays.stream(intArr).count();     //보통은 필터링후 count 개수 구함
//        System.out.println(count);
////        optional객체 : 값이 있을수도 있고, 없을수도 있음을 명시한 객체
//        int max = Arrays.stream(intArr).max().getAsInt();   //intArr값이 있는걸 아니까 이렇게 설정 getAsInt(알고리즘 문제풀이에는 보통 getAsInt사용)
//        int min = Arrays.stream(intArr).min();.getAsInt()
//        int max = Arrays.stream(intArr).max().orElseThrow;  //intArr이 비어있을수 있다면 max값이 없으므로 리턴값에 optionalInt값(실무에서 반드시 사용)

////        reduce : 누적연산 -> reduce(초기값, 연산식)
//        int[] intArr = {10,20,30,40};
//        int accSum = Arrays.stream(intArr).reduce(0,(a, b)->a+b);  //a에 값이 계속 누적
...
////      배열로 변환시 주의사항(안중요함) : 제네릭의 타입소거 -> 자바의 런타임시점에 <String>과 같은 제네릭의 타입이 제거되는 현상
////      제네릭의 타입소거로 인해, toArray를 통해 새로운 String배열을 곧바로 만들어 내는 것은 불가
//        String[] stArr = {"hello", "java","world"};
////        String[] answer = Arrays.stream(stArr).filter( a->a.length()>=5).toArray();
...
////        Optional객체 처리 방법 4가지 (중요**)
////        방법1. isPresent()로 확인후 get()
//        if(opt1.isPresent()){
...
////        웹개발에서 값을 입력하는 사용자에게 적절한 메서지 전달 목적과 의도된 코드중단을 목표로 강제로 예외(에러)를 발생시키는 경우는 매우 일반적
...
//        Student s1 = studentList.get(number);   //없는 번호를 입력받게되면 에러가 뜸
