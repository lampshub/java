...
//
////        클래스메서드 사용 예시
//        System.out.println(Math.min(10,20));    //객체(자신만의 메모리공간)가 따로 필요없음
////        객체 사용 예시
//        List<Integer> myList = new ArrayList<>();
//        myList.add(10);
//        myList.add(20);
        System.out.println(Calculator.sumAcc(30));
        System.out.println(Calculator.sumAcc(30));
        System.out.println(Calculator.total);

        CalculatorForInstance c1 = new CalculatorForInstance();
        System.out.println(c1.sumAcc(30));
        System.out.println(c1.sumAcc(30));
    ...
    class Calculator {
//    클래스에 변수선언 1. static이 붙어있으면 클래스변수 2. 붙어있지않으면 객체변수(인스턴스 변수)
        static int total = 0;
//    클래스메서드선언
...
//    클래스에 객체변수선언
    int total = 0;
//    메서드에 static 있는 경우는 클래스메서드
//    클래스메서드는 일반적으로 객체의 상태와 상관없는 독립적인 로직 수행시에 사용
//    클래스메서드 호출시 : 클래스명.메서드명();
    public static int sum(int a, int b) {
        return a+b;     //연산해서 뱉어내는 경우이므로, 객체메서드로 사용할 필요 없음.
    }
//    메서드에 static이 없는 경우는 객체메서드
//    객체메서드 호출시 : 객체명.메서드명();   //객체를 먼저 생성후 호출
    public int sumAcc(int a) {
//        this는 객체를 지칭. 일반적으로 매개변수와 객체변수 이름을 구분짓기 위해 사용 (여기서는 a와 total)
        this.total += a;
        return this.total;
    }
}
